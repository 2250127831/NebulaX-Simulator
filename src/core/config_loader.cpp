...
Config ConfigLoader::load(const std::string& path) {
    ...
    // strategy（吸收自 Trader：白名单 + 加权净投票仲裁配置）
    ...
    // replay（本仿真新增）
    if (auto n = root["replay"]) {
        if (n["file"])           cfg.replay.file          = n["file"].as<std::string>();
        if (n["segment_size"])   cfg.replay.segment_size  = n["segment_size"].as<size_t>();
        if (n["max_messages"])   cfg.replay.max_messages  = n["max_messages"].as<size_t>();
        if (n["order_id_base"])  cfg.replay.order_id_base = n["order_id_base"].as<uint64_t>();
    }

    // analysis（本仿真新增）
    if (auto n = root["analysis"]) {
        if (n["out_dir"]) cfg.analysis.out_dir = n["out_dir"].as<std::string>();
    }
