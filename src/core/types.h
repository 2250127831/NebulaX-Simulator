...
// 精度（本仿真统一）：
//   - 全链路（引擎 / 策略簿 / 风控 / OMS / 配置资金）统一用 ITCH 原始精度
//     ×10000（tick，1/10000 元）。100 ticks = 1 分，故分制值 ×100 即 ticks。
//   - PnL/净值单位 = tick × 股（1/10000 元 × 股），配置里的资金值同为该单位。
// 换算只发生在协议边界和展示层，见 price_to_double()。

// ── Tick 数据结构 ──
// 定长设计，避免变长消息带来的内存管理开销
struct Tick {
    // 本仿真全链路统一价格精度 = ITCH 原始精度 1/10000 元（tick）。
    // 100 ticks = 1 分，故 1/100（分）口径完全兼容（×100 转 ticks）。
    static constexpr int64_t kTickSize = 10000;   // 价格精度：1 元 = 10000 ticks

    uint64_t timestamp;       // 时间戳（纳秒）
    uint64_t seq_id;          // 行情序列号
    uint64_t symbol_id;       // 合约/股票 ID
    int64_t   last_price;     // 最新成交价（tick，定点整数）
    uint64_t volume;          // 成交量
    int64_t   bid_price;      // 买一价（tick，定点整数）
    int64_t   ask_price;      // 卖一价（tick，定点整数）
