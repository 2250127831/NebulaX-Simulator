...
// SimWorker::on_market_event 每条事件算一次 side + BBO + 现价, 填进 ctx 传所有策略。
...
// （契约吸收自 NebulaX-Trader strategy/base/strategy.h，CRTP 编译期多态版）
struct BookContext {
...
// on_segment(seg) 段边界钩子(Simulator 的插入窗口): 基类默认 no-op, 需要者覆写。
...
public:
    ...
    void reset() {}
    void on_segment(uint64_t /*segment*/) {}
};
